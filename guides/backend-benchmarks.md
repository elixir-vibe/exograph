...
A local QuackDB prototype optimized ordinary signed 64-bit vector encoding, including large `BIGINT[]` child vectors. Dogfooding it through a temporary local path dependency produced a fixed top500 run with `379 indexed / 121 skipped / 0 failed`, `91.6s` elapsed, and `fragment_append_rows` at `78.8s` (`stage_rows` 28.7s, `merge_query` 20.0s). The run was promising but not enough by itself to justify a QuackDB release; keep collecting adapter/protocol improvements. Exograph was restored to the released `~> 0.5.11` dependency after the run.
