...
2. **Increase ingestion flush granularity**
   ...
   - Local QuackDB query-phase telemetry dogfood split query/fetch encode, transport, decode, normalize, and bytes. Fixed top500 showed aggregated `INSERT` query time is almost entirely transport/server execution (`63.0s` query duration, `62.8s` transport) while fetch/materialization is small (`0.43s`, `5.2 MB`). The remaining app-side `fragment_append_merge_query` gap likely sits in server-side DuckDB execution/storage/checkpoint behavior, temp-stage specifics, or native append ingestion rather than client-side query row materialization.
   ...
   - A local query-telemetry run with `--concurrency 1` reduced MERGE query time from ~27.1s to ~8.3s and `INSERT` query transport from ~62.8s to ~24.3s, but regressed wall time to ~251s by serializing extraction/indexing. Next target: keep extraction/package concurrency high but bound DuckDB fragment append/write concurrency (e.g. a dedicated writer or fragment-append semaphore) instead of serializing the whole corpus.
