...
3. **Prototype an offline build/finalize schema**
   ...
   - It can also stage terms, definitions, references, comments, and fragment_terms with `fragment_content_hash` and bind them to final IDs during finalization.
   - Next work: extend the same idea to files and call graph facts, then wire a full package/corpus path for benchmarking.
