...
2. **Increase ingestion flush granularity**
   ...
   - Fetch telemetry plus an offline DuckDB CLI MERGE profile showed MERGE `RETURNING` result fetch is small (~0.55s), while a comparable CLI MERGE-with-RETURNING operator profile was only ~3.5s latency / ~2.6s CPU. The remaining app-side `fragment_append_merge_query` gap likely sits in Quack request/response, server-side prepare/materialization, checkpoint/storage effects, or temp-stage specifics. Next: add finer QuackDB query-phase telemetry (prepare transport/decode, fetch transport/decode/materialization, response bytes) rather than more prelookup/direct-insert variants.
