...
2. **Increase ingestion flush granularity**
   ...
   - Per-column payload attribution (`--duckdb-fragment-payload-metrics`) showed the fragment staging request is dominated by `ast` (~452.6 MB), `terms` (~234.1 MB), and `sub_hashes` (~56.4 MB) in fixed top500. Next likely win: prototype a two-phase narrow-key staging path that avoids sending full AST/list payloads through the MERGE stage for already-existing hashes.
