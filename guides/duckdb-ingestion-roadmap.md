...
2. **Increase ingestion flush granularity**
   ...
   - A direct Exograph-side `QuackDB.insert_columns/4` bypass failed because it used a different QuackDB connection than the surrounding `repo.transaction/2`, making the temp table invisible. Pursue this only via QuackDB/Ecto adapter support for active-transaction native append, or by redesigning staging away from connection-local temp tables.
